...
mod packed_coordinate_sequences_tests {
    ...
    fn test_copy_to_larger_dim() {
        ...
        let mut cs3d = PackedCoordinateSequenceFactory::create_with_size_dimension(10, 3);
        let cs3d_size = cs3d.size();
        PackedCoordinateSequences::copy(&cs2d, 0, &mut cs3d, 0, cs3d_size);
        assert!(PackedCoordinateSequences::is_equal(&cs2d, &cs3d));
    }

    #[test]
    fn test_copy_to_smaller_dim() {
        let cs3d = create_test_sequence(10, 3);
        let mut cs2d = PackedCoordinateSequenceFactory::create_with_size_dimension(10, 2);
        let cs2d_size = cs2d.size();
        PackedCoordinateSequences::copy(&cs3d, 0, &mut cs2d, 0, cs2d_size);
        assert!(PackedCoordinateSequences::is_equal(&cs2d, &cs3d));
    }
    ...
    fn create_sequence_from_ordinates(dim: i32) -> PackedCoordinateSequenceDouble {
        let mut sequence =
            PackedCoordinateSequenceFactory::create_with_size_dimension(ORDINATE_VALUES.len(), dim);
    ...
    fn create_test_sequence(size: usize, dim: i32) -> PackedCoordinateSequenceDouble {
        let mut cs = PackedCoordinateSequenceFactory::create_with_size_dimension(size, dim);
    ...
    fn do_test_reverse(dimension: i32) {
        ...
        // act
        PackedCoordinateSequences::reverse(&mut reversed);
    ...
    fn do_test_copy(dimension: i32) {
        ...
        let mut full_copy =
            PackedCoordinateSequenceFactory::create_with_size_dimension(sequence.size(), dimension);
        let mut partial_copy = PackedCoordinateSequenceFactory::create_with_size_dimension(
            sequence.size() - 5,
            dimension,
        );

        // act
        PackedCoordinateSequences::copy(&sequence, 0, &mut full_copy, 0, sequence.size());
        let partial_copy_size = partial_copy.size();
        PackedCoordinateSequences::copy(&sequence, 2, &mut partial_copy, 0, partial_copy_size);
    ...
    fn do_test_is_ring(dimension: i32) {
        ...
        // TODO: Fix me!
        // let incomplete4b = PackedCoordinateSequences::ensure_valid_ring(&incomplete4a);

        // act
        let is_ring_ring = PackedCoordinateSequences::is_ring(&ring);
        let is_ring_no_ring = PackedCoordinateSequences::is_ring(&no_ring);
        let is_ring_empty = PackedCoordinateSequences::is_ring(&empty);
        let is_ring_incomplete1 = PackedCoordinateSequences::is_ring(&incomplete1);
        let is_ring_incomplete2 = PackedCoordinateSequences::is_ring(&incomplete2);
        let is_ring_incomplete3 = PackedCoordinateSequences::is_ring(&incomplete3);
        let is_ring_incomplete4a = PackedCoordinateSequences::is_ring(&incomplete4a);
        // TODO: Fix me!
        // let is_ring_incomplete4b = PackedCoordinateSequences::is_ring(&incomplete4b);
    ...
    fn do_test_index_of(dimension: i32) {
        ...
        for i in 0..sequence.size() {
            assert_eq!(
                i,
                PackedCoordinateSequences::index_of(&coordinates[i], &sequence) as usize
            );
        }
    }
    ...
    fn do_test_min_coordinate_index(dimension: i32) {
        ...
        assert_eq!(
            min_index,
            PackedCoordinateSequences::min_coordinate_index_sequence(&sequence)
        );
        assert_eq!(
            min_index,
            PackedCoordinateSequences::min_coordinate_index_sequence_from_to(
                &sequence,
                2,
                sequence.size() - 2
            )
    ...
    fn do_test_scroll(dimension: i32) {
        ...
        // act
        PackedCoordinateSequences::scroll_coordinate_index(&mut scrolled, 12);
    ...
    fn check_coordinate_at(
        seq1: &PackedCoordinateSequenceDouble,
        pos1: usize,
        seq2: &PackedCoordinateSequenceDouble,
        pos2: usize,
        dim: i32,
    ) {
    ...
    fn create_almost_ring(dimension: i32, mut num: usize) -> PackedCoordinateSequenceDouble {
        if num > 4 {
            num = 4;
        }

        let mut sequence =
            PackedCoordinateSequenceFactory::create_with_size_dimension(num, dimension);
        if num == 0 {
            return fill_non_planar_dimensions(&sequence);
        }
    ...
    fn create_circular_string(
        ...
        let mut sequence = PackedCoordinateSequenceFactory::create_with_size_dimension(
            num_points as usize,
            dimension,
        );
